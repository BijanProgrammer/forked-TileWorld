class Ball extends Element {
  ...
  randomMove() {
    ...
    const closeBallsLocations = BALLS.filter((item) => {
      ...
      return Math.abs(location[0] - this.row) + Math.abs(location[1] - this.column) === 1 && !this.isArrived;
    ...
    if (closeBallsLocations.length !== 0) {
      directions = directions.filter((direct) => {
        for (let index = 0; index < closeBallsLocations.length; index++) {
          ...
          if (location[0] === direct[0] && location[1] === direct[1]) return false;
        }
        return true;
      });
    ...
    let randomLocation = directions[Math.floor(Math.random() * directions.length)];
    ...
    const holes = HOLES.filter((hole) => hole.rowCol[0] === this.row && hole.rowCol[1] === this.column && !hole.isFill);
    if (holes.length !== 0) {
      holes[0].isFill = true;
      this.isArrived = true;
