...
function readyGame() {
  ...
  for (let i = 0; i < team1PlayerCount + team2PlayerCount; i++) {
    ...
    agents[i] = new Agent(randomNumbers[i + 2 * ballsCount], startFule, team);
...
class Element {
  ...
  getElement() {
    ...
    element.style.top = (this.row + 1) * cellSize - (cellSize / 2 + cellSize / (this.SIZE_RATIO * 2)) + 'px';
    element.style.left = (this.column + 1) * cellSize - (cellSize / 2 + cellSize / (this.SIZE_RATIO * 2)) + 'px';
...
function manageLogs() {
  ...
  const ballsLocationString = BALLS.map((item) => `(${item.rowCol.join(', ')})`).join(' ');
  const holesLocationString = HOLES.map((item) => `(${item.rowCol.join(', ')})`).join(' ');
  const agentsLocationString = AGENTS.map(
    (item) => `((${item.rowCol.join(', ')}), fule: ${item.fule} , direction: ${item.direction})`
  ).join(' ');
